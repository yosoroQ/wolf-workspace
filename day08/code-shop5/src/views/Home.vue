<template>
  <div class="home">
    ...
    <div class="content">
      <div class="wrap">
        <JfTitle title="精品推荐" :imgSrc="titImg1"></JfTitle>
        <List :arr="jingpinArr"></List>
        <JfTitle title="热门兑换" :imgSrc="titImg2"></JfTitle>
        <img
    ...
    <div class="wrap">
      <JfTitle title="积分攻略" :imgSrc="titImg3"></JfTitle>
      <ul class="jifen">
...
export default {
  ...
  async created() {
    //请求精品4条 和 热门数据8条 显示到页面中.
    let res1 = await JingpinAPI();
    console.log("res1:", res1.data.data.records);
    // 获取的数据可能超过4条，可以使用filter()方法进行筛选得到前4条的数据
    this.jingpinArr = res1.data.data.records.filter((item, index) => index < 4);
    let res2 = await HotAPI();
    console.log("res2:", res2.data.data.records);
    this.remenArr = res2.data.data.records.filter((item, index) => index < 10);
  },
};
</script>
